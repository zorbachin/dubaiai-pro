...
export async function sendAuditEmail(opts: {
  ...
  const subject = `I found 5 places AI could save or make you money — ${opts.report.business_name}`;
...
<html><body style="font-family: -apple-system, Segoe UI, Roboto, sans-serif; background:#0b0d12; color:#e7eaf0; padding:24px;">
  <div style="max-width:640px; margin:0 auto; background:#11141b; border:1px solid #1f2330; border-radius:14px; padding:28px;">
    <h1 style="margin:0 0 6px; font-size:22px;">I found 5 places AI could save or make you money at ${escapeHtml(r.business_name)}.</h1>
    <div style="color:#8a93a6; font-size:14px;">${escapeHtml(getDomain(r.website_url))}</div>
    <p style="margin:14px 0 0; color:#e7eaf0;">Public-only audit. Every claim cites a source, and anything we couldn't verify is marked as such.</p>
    ...
    <h2 style="font-size:16px; margin:18px 0 8px;">Top 3 opportunities</h2>
    ...
    <p style="color:#8a93a6; font-size:13px;">Reply to this email to book a 20-minute walkthrough.</p>
  </div>
</body></html>`;
}
