...
<!-- HANDOFFS:START -->
### 2026-06-07 17:18 UTC · code
HERMES SETUP BUILT (PR #68 on claude/hermes-setup-YnHMp): bin/hermes-setup = one-command Mac installer (clones NousResearch/hermes-agent, installs via uv, applies config, prompts API key + Telegram token). Token-drain fix: Sonnet model not Opus, cron OFF, max 25 turns, aggressive compression. bin/hermes-start = background Telegram gateway. .hermes-config/ = config template + system prompt + .env template. To finish: pull branch, run bin/hermes-setup (interactive), then bin/hermes-start.
